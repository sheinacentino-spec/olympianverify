...
function subscriberSearch(){
  ...
  out.innerHTML = '';
  if(!q){ out.innerHTML = '<div class="muted">Enter a name or ID to verify.</div>'; return; }

  const found = employees.find(e => e.id.toLowerCase() === q || e.name.toLowerCase().includes(q));
  if(!found){ out.innerHTML = '<div class="muted">No technician found.</div>'; return; }
...
function openAdminLoginModal(){
  ...
  overlay.innerHTML = `
    <div class="modal-card">
      ...
      <input id="adminPasswordInput" type="password" placeholder="Enter Password" style="width:80%;padding:10px;margin-top:10px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:white">
...
function adminAddEmployee(){
  ...
  reader.onload = e => {
    ...
    const qr = new QRious({ value: window.location.href.split('?')[0] + '?id=' + encodeURIComponent(id), size: 160 });
